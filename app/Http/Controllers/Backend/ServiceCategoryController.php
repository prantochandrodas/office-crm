...
class ServiceCategoryController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ->addColumn('action', function ($row) {
                    ...
                    $editUrl = route('service-categories.edit', $row->id);
                    $deleteUrl = route('service-categories.distroy', $row->id);
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('service-categories')->with('success', 'Data added successfull');
    }
    ...
    public function edit($id)
    {
        if (auth()->check()) {
            if (auth()->user()->can('service-category-edit')) {
                ...
                return view('backend.service_category.edit', compact('data'));
            } else {
                auth()->logout(); // Log out the user
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('service-categories')->with('success', 'Data updated successfully');
    }
    ...
    public function distroy($id)
    {
        if (auth()->check()) {
            if (auth()->user()->can('service-category-delete')) {
                ...
                $data->delete();
                // Redirect back with success message
                return redirect()->route('service-categories')->with('success', 'Data deleted successfully');
            } else {
                auth()->logout(); // Log out the user
                return redirect()->route('login')->with('error', 'You do not have permission to create and have been logged out.');
            }
