...
class NonProspectiveClientController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ->addColumn('action', function ($row) {
                    ...
                    $editUrl = route('non-prospective-clients.edit', $row->id);
                    $deleteUrl = route('non-prospective-clients.distroy', $row->id);
                    $csrfToken = csrf_field();
                    $method = method_field('DELETE');

                    if (auth()->user()->can('add-to-primary-client')) {
                    ...
                    if (auth()->user()->can('non-prospective-client-delete')) {
    ...
    public function edit($id)
    {
        ...
        if (auth()->check()) {
            if (auth()->user()->can('non-prospective-client-edit')) {
                ...
                return view('backend.non_prospective_client.edit', compact('customer', 'projects', 'locations', 'divisions', 'districts', 'locations', 'selectedDivisionId', 'selectedDistrictId', 'selectedLocationId', 'serviceCategory'));
            } else {
                auth()->logout(); // Log out the user
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('non-prospective-clients')->with('success', 'Data updated successfully');
    }
    ...
    {
        if (auth()->check()) {
            if (auth()->user()->can('non-prospective-client-distroy')) {
                ...
                $customer->delete();
                // Redirect back with success message
                return redirect()->route('non-prospective-clients')->with('success', 'Data deleted successfully');
            } else {
                auth()->logout(); // Log out the user
                return redirect()->route('login')->with('error', 'You do not have permission to view and have been logged out.');
            }
