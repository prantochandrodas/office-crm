...
class WantedClientController extends Controller
{
    public function index()
    {
        if (auth()->check()) {
            if (auth()->user()->can('wanted-client')) {
                return view('backend.wanted_client.index');
            } else {
    ...
    {
        if ($request->ajax()) {
            $data = Customer::where('status', 2)->orderBy('created_at', 'desc')->get();
            return DataTables::of($data)
                ->addColumn('action', function ($row) {
                    ...
                    $editUrl = route('wanted-clients.edit', $row->id);
                    $deleteUrl = route('wanted-clients.distroy', $row->id);
                    $csrfToken = csrf_field();
                    $method = method_field('DELETE');

                    $addOurClient = '<span data-id="' . $row->id . '" style="cursor:pointer; padding:10px; font-size:13px" class="add-our-client badge rounded-pill text-bg-primary text-light ms-2">add to our-client</span>';
                    ...
                    if (auth()->user()->can('wanted-client-edit')) {
                    ...
                    if (auth()->user()->can('wanted-client-distroy')) {
    ...
    public function edit($id)
    {
        if (auth()->check()) {
            if (auth()->user()->can('wanted-client-edit')) {
                ...
                return view('backend.wanted_client.edit',  compact('customer', 'projects', 'locations', 'divisions', 'districts', 'locations', 'selectedDivisionId', 'selectedDistrictId', 'selectedLocationId', 'serviceCategory'));
            } else {
                auth()->logout(); // Log out the user
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('wanted-clients')->with('success', 'Data updated successfully');
    }
    ...
    {
        if (auth()->check()) {
            if (auth()->user()->can('wanted-client-delete')) {
                $customer = Customer::findOrFail($id);
                CustomerProject::where('customer_id', $customer->id)->delete();
                $customer->delete();
                return redirect()->route('wanted-clients')->with('success', 'Data deleted successfully');
            } else {
                auth()->logout(); // Log out the user
                return redirect()->route('login')->with('error', 'You do not have permission to distroy and have been logged out.');
            }
        } else {
            return redirect()->back()->with('error', 'You need to login first.');
        }
