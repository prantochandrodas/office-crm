...
class ContactClientController extends Controller
{
    ...
    {
        if (auth()->check()) {
            if (auth()->user()->can('contact-client')) {
                $divisions = Division::all();
                $serviceCategories = ServiceCategory::all();
                return view('backend.contact_client.index',compact('divisions', 'serviceCategories'));
            } else {
        ...
        if ($request->ajax()) {
            return DataTables::of($customers)
                ->addColumn('action', function ($row) {
                    ...
                    if (auth()->user()->can('add-to-wanted-client')) {
                    ...
                    if (auth()->user()->can('contact-client-delete')) {
    ...
    public function edit($id)
    {
        ...
        if (auth()->check()) {
            if (auth()->user()->can('contact-client-edit')) {
                ...
                return view('backend.contact_client.edit', compact('customer', 'projects', 'locations', 'divisions', 'districts', 'locations', 'selectedDivisionId', 'selectedDistrictId', 'selectedLocationId', 'serviceCategory'));
            } else {
                auth()->logout(); // Log out the user
    ...
    {
        if (auth()->check()) {
            if (auth()->user()->can('contact-client-distroy')) {
                $customer = Customer::findOrFail($id);
