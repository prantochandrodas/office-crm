...
class OurClientController extends Controller
{
    ...
    {
        if (auth()->check()) {
            if (auth()->user()->can('our-client')) {
                $divisions = Division::all();
                $serviceCategories = ServiceCategory::all();
                return view('backend.our_client.index',compact('divisions','serviceCategories'));
            } else {
        ...
        if ($request->ajax()) {
            return DataTables::of($customers)
                ->addColumn('action', function ($row) {
                    ...
                    $editUrl = route('our-clients.edit', $row->id);
                    $deleteUrl = route('our-clients.distroy', $row->id);
                    ...
                    if (auth()->user()->can('our-client-edit')) {
    ...
    public function edit($id)
    {
        if (auth()->check()) {
            if (auth()->user()->can('our-client-edit')) {
                ...
                return view('backend.our_client.edit', compact('customer', 'projects', 'locations', 'divisions', 'districts', 'locations', 'selectedDivisionId', 'selectedDistrictId', 'selectedLocationId', 'serviceCategory'));
            } else {
                auth()->logout(); // Log out the user
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('our-clients')->with('success', 'Data updated successfully');
    }
    ...
    {
        if (auth()->check()) {
            if (auth()->user()->can('our-client-delete')) {
                $customer = Customer::findOrFail($id);
                CustomerProject::where('customer_id', $customer->id)->delete();
                $customer->delete();
                return redirect()->route('our-clients')->with('success', 'Data deleted successfully');
            } else {
                auth()->logout(); // Log out the user
